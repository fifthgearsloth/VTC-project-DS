    ...
    if run:
        if not goal_input and not style_input and not persona_input:
            ...
            with st.spinner("Finding your best trainer matches..."):
                ...
                jaccard_df = pd.DataFrame(jaccard_results).sort_values("final_score", ascending=False).head(3).reset_index(drop=True)
                jaccard_df["rank"] = jaccard_df.index + 1
            ...
            def generate_why(goal_score, style_score, persona_score, trainer):
                reasons = []
                if goal_score >= 0.5:
                    reasons.append(f"your goals align closely with this trainer's specialisation in {trainer['goal_tags'][:50]}")
                if style_score >= 0.5:
